...
export default function Footer() {
  ...
  const year = siteConfig.brand.foundingYear

  const t =
    locale === "en"
      ? {
          description: siteConfig.footer.descriptionEn,
          slogan: siteConfig.brand.sloganEn,
          productsHeading: "Products",
          products: [...siteConfig.footer.productCategoriesEn],
          ...
          copyright: `© ${year} ${siteConfig.brand.name}. All rights reserved.`,
          privacyPolicy: "Privacy Policy",
          terms: "Terms & Conditions",
        }
      : {
          description: siteConfig.footer.descriptionBn,
          slogan: siteConfig.brand.slogan,
          productsHeading: "পণ্য বিভাগ",
          products: [...siteConfig.footer.productCategoriesBn],
          ...
          copyright: `© ${year} ${siteConfig.brand.name}. সর্বস্বত্ব সংরক্ষিত।`,
        ...
        <div className="grid grid-cols-3 md:grid-cols-4 gap-4 md:gap-6">
          ...
          <div>
            ...
            <div className="flex flex-col gap-1.5 text-sm text-black">
              {t.products.map((p) => (
                <Link key={p} href={href("/shop")} className="hover:text-green-700 transition">
                  {p}
                </Link>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="flex flex-col gap-1.5 text-sm text-black">
              <Link href={href("/customer/dashboard")} className="hover:text-green-700 transition">
                {t.trackOrder}
              </Link>
              <Link href={href("/return-policy")} className="hover:text-green-700 transition">
                {t.returnPolicy}
              </Link>
              <Link href={href("/faq")} className="hover:text-green-700 transition">
                {t.faq}
              </Link>
              <Link href={href("/contact")} className="hover:text-green-700 transition">
                {t.contact}
              </Link>
            </div>
          </div>
          ...
          <div className="col-span-2 md:col-span-1">
            ...
            <div className="flex flex-col gap-1.5 text-sm text-black">
              ...
              <a href={`tel:${siteConfig.contact.phone}`} className="hover:text-green-700 transition">
              ...
              {siteConfig.contact.businessWhatsapp ? (
                <a
                  href={`https://wa.me/${siteConfig.contact.businessWhatsapp}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="hover:text-green-700 transition"
                >
                  💼 {t.businessContact}: {siteConfig.contact.businessPhoneDisplay}
                </a>
              ) : null}
            </div>
          </div>
      ...
      <div className="bg-green-800 text-white border-t border-green-700 py-4 mt-2">
        <div className="max-w-7xl mx-auto px-4 flex flex-col md:flex-row justify-between items-center text-xs text-green-200 gap-2">
          ...
          <div className="flex gap-4">
            <Link href={href("/privacy-policy")} className="hover:text-yellow-400 transition">
              {t.privacyPolicy}
            </Link>
            <Link href={href("/terms")} className="hover:text-yellow-400 transition">
              {t.terms}
            </Link>
            <Link href={href("/return-policy")} className="hover:text-yellow-400 transition">
              {t.returnPolicy}
            </Link>
          </div>
        </div>
