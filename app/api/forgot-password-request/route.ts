...
export async function POST(request: Request) {
  try {
    ...
    if (!rateCheck.allowed) {
      ...
      return NextResponse.json(
        { error: `অনেকবার request করা হয়েছে। ${minutes} মিনিট পর আবার চেষ্টা করুন।` },
        { status: 429 }
      )
    }
    ...
    if (!customer) {
      return NextResponse.json(
        { error: "এই নম্বরে কোনো অ্যাকাউন্ট বা অর্ডার পাওয়া যায়নি" },
        { status: 404 }
      )
    }
    ...
    return NextResponse.json({
      success: true,
      message: `আপনার রিকোয়েস্ট পাঠানো হয়েছে। আমাদের টিম শীঘ্রই আপনাকে কল করবে, অথবা সরাসরি যোগাযোগ করুন: ${siteConfig.contact.phoneDisplay}`,
    })
  } catch (error) {
