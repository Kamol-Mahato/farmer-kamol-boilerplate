...
// 🔒 থি রুট কখনো CDN/ব্রাউজারে ক্যাশ হলে দুই ভিজিটর একই visitorId পেতে পারে
...
function buildWelcomeText() {
  const { brand, address, contact, social } = siteConfig
  return (
    `${brand.name}-ে আপনাকে স্বাগতম 🌿\n\n` +
    `আমরা ${address.region}ের ${address.locality} থেকে সরাসরি খাঁটি ও মানসম্মত দেশি পণ্য পৌঁছে দিচ্ছি আপনার দরজায়।\n\n` +
    `আপনাকে কীভাবে সাহায্য করতে পারি? আপনার প্রশ্নটি নিচে লিখে দিন, আমাদের প্রতিনিধি শীঘ্রই আপনার সাথে যুক্ত হবেন।\n\n` +
    `📞 দ্রুত উত্তরের জন্য কল বা হোয়াটসঅপ করুন: ${contact.phoneDisplay}\n\n` +
...
export async function GET() {
  try {
    ...
    if (!conversation) {
      return json({ error: "চ্যাট শুরু করতে সমস্যা হয়েছে" }, 500)
    }
